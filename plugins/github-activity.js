...
module.exports = GitHubActivity = (options) => {
  return new Promise((resolve, reject) => {
    ...
    var methods = {
      ...
      getOutputFromRequest: function (url, callback) {
        ...
        request.onerror = function (e) { console.log('error', e), callback('An error occurred connecting to ' + url); };
        request.send();
      },
    ...
    methods.getOutputFromRequest(userUrl, function (error, output) {
      ...
      return resolve(Mustache.render(templates.Stream, { text: header + activity, footer: templates.Footer }));
    });
    /*
    ...
    methods.getOutputFromRequest(eventsUrl, function(error, output) {
      ...
      if (error) {
        ...
        activity = methods.getActivityHTML(output, 20);
      }
